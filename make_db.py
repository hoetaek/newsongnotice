...
def make_db():
    ...
    c.execute(
        "CREATE TABLE IF NOT EXISTS pop ( `id` INTEGER PRIMARY KEY AUTOINCREMENT, `POP` TEXT )"
    )
    c.execute(
        "CREATE TABLE IF NOT EXISTS users_pop ( `user_id` INTEGER, `pop_id` INTEGER, FOREIGN KEY(`user_id`)"
        " REFERENCES `users`(`id`), FOREIGN KEY(`pop_id`) REFERENCES `pop`(`id`) )"
    )
...
def insert_query():
    ...
    c.execute("SELECT * FROM pop WHERE POP = '신청'")
    kpop_entry = c.fetchone()
    if not kpop_entry:
        c.execute("INSERT INTO pop VALUES(NULL, '{}')".format("신청"))
    conn.commit()
    c.close()
    conn.close()
...
def insert_user_pop(c, user):
    ...
    c.execute("INSERT INTO users_pop VALUES({}, {})".format(user_id, '1'))
...
def user_list(c, genre):
    ...
    if genre == 'kpop':
        ...
        c.execute(
            "SELECT user FROM users, pop, users_pop WHERE pop.id = users_pop.pop_id AND"
            " users.id = users_pop.user_id AND POP = '{}'".format('신청')
        )
        user = [row[0] for row in c.fetchall()]
    return user
